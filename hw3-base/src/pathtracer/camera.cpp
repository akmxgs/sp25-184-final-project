...
Ray Camera::generate_ray(double x, double y) const {
  ...
  Vector3D bottom_left = Vector3D(-std::tan(radians(hFov) * 0.5), -std::tan(radians(vFov) * 0.5), -1);
  Vector3D top_right = Vector3D(std::tan(radians(hFov) * 0.5), std::tan(radians(vFov) * 0.5), -1);

  double X = top_right.x * 2 * x + bottom_left.x ; // calculate position of coords
  double Y = top_right.y * 2 * y + bottom_left.y;
  double Z = -1;

  Vector3D direction = c2w * Vector3D(X, Y, Z); // convert ray to world space
  direction.normalize();
